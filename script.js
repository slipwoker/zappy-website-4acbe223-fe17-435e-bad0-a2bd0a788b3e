...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        console.log('⚠️ Zappy: Skipping API call - handler not properly loaded');
      } else {
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '4acbe223-fe17-435e-bad0-a2bd0a788b3e',
                name: data.name || '',
                email: data.email || '',
            ...
            // Don't break existing functionality - silently fail
          }
        })();
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "גיליה הרוחנית משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
